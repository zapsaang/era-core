...
impl LsmChunkIndex {
    ...
    pub fn iter(
        &self,
    ) -> impl Iterator<Item = Result<(ChunkHash, BlockLocation), IndexError>> + '_ {
        self.db
    ...
    fn serialize_location(location: &BlockLocation) -> Result<Vec<u8>, IndexError> {
        let proto: era_common::proto::BlockLocation = location.clone().into();
        era_common::serialize_proto(&proto).map_err(|e| IndexError::Serialization(e.to_string()))
    }

    /// Deserialize a BlockLocation from bytes.
    fn deserialize_location(bytes: &[u8]) -> Result<BlockLocation, IndexError> {
        let proto: era_common::proto::BlockLocation = era_common::deserialize_proto(bytes)
            .map_err(|e| IndexError::Deserialization(e.to_string()))?;
        proto
            .try_into()
            .map_err(|e| IndexError::Deserialization(format!("Conversion error: {}", e)))
    }
