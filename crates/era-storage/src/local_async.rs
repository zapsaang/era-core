...
//! Adheres to the "Zero-Blocking Mandate" - no sync I/O in async contexts.
...
use crate::traits::{AsyncStorageBackend, AsyncStorageReader, AsyncStorageWriter, StorageMetadata};

/// Async local filesystem storage backend
#[derive(Debug, Clone)]
pub struct AsyncLocalStorageBackend {
    /// Base directory for storage
    base_path: PathBuf,
}

impl AsyncLocalStorageBackend {
    /// Create a new async local storage backend
...
#[async_trait]
impl AsyncStorageBackend for AsyncLocalStorageBackend {
    type Writer = AsyncLocalStorageWriter;
    type Reader = AsyncLocalStorageReader;
    ...
    async fn create(&self, path: &Path) -> Result<Self::Writer> {
        ...
        Ok(AsyncLocalStorageWriter { file, size: 0 })
    }
    ...
    async fn open_append(&self, path: &Path) -> Result<Self::Writer> {
        ...
        Ok(AsyncLocalStorageWriter { file, size })
    }
    ...
    async fn open_read(&self, path: &Path) -> Result<Self::Reader> {
        ...
        Ok(AsyncLocalStorageReader { file, size })
    }
...
/// Async writer for local filesystem storage
pub struct AsyncLocalStorageWriter {
    file: File,
    size: u64,
}

#[async_trait]
impl AsyncStorageWriter for AsyncLocalStorageWriter {
...
/// Async reader for local filesystem storage
pub struct AsyncLocalStorageReader {
    file: File,
    size: u64,
}

#[async_trait]
impl AsyncStorageReader for AsyncLocalStorageReader {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_async_local_storage_create_and_write() {
    ...
    #[tokio::test]
    async fn test_async_local_storage_read() {
        let temp_dir = TempDir::new().unwrap();
        let backend = AsyncLocalStorageBackend::new(temp_dir.path());
    ...
    #[tokio::test]
    async fn test_async_local_storage_write_at() {
        let temp_dir = TempDir::new().unwrap();
        let backend = AsyncLocalStorageBackend::new(temp_dir.path());
    ...
    #[tokio::test]
    async fn test_async_local_storage_truncate() {
        let temp_dir = TempDir::new().unwrap();
        let backend = AsyncLocalStorageBackend::new(temp_dir.path());
    ...
    #[tokio::test]
    async fn test_async_local_storage_exists() {
        let temp_dir = TempDir::new().unwrap();
        let backend = AsyncLocalStorageBackend::new(temp_dir.path());
    ...
    #[tokio::test]
    async fn test_async_local_storage_delete() {
        let temp_dir = TempDir::new().unwrap();
        let backend = AsyncLocalStorageBackend::new(temp_dir.path());
