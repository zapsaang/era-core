...
pub const CHECKPOINT_EXTENSION: &str = ".era.checkpoint";

/// Extension for temporary checkpoint files (during atomic write)
pub const CHECKPOINT_TMP_EXTENSION: &str = ".era.checkpoint.tmp";

/// Custom serde module to serialize HashMap<ChunkHash, BlockLocation> as a Vec
mod chunk_map_serde {
    use super::*;
    ...
    pub fn serialize<S>(
        map: &HashMap<ChunkHash, BlockLocation>,
        serializer: S,
    ) -> std::result::Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let entries: Vec<ChunkEntry> = map
            .iter()
            .map(|(hash, location)| ChunkEntry {
                hash: *hash,
                location: *location,
            })
            .collect();
    ...
    pub fn deserialize<'de, D>(
        deserializer: D,
    ) -> std::result::Result<HashMap<ChunkHash, BlockLocation>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let entries: Vec<ChunkEntry> = Vec::deserialize(deserializer)?;
        Ok(entries.into_iter().map(|e| (e.hash, e.location)).collect())
    }
}
...
pub struct Checkpoint {
    ...
    /// All chunk hashes that have been written (stored as Vec for JSON compatibility)
    #[serde(with = "chunk_map_serde")]
...
impl Checkpoint {
    ...
    /// Get the checkpoint file path for an archive
...
impl CheckpointManager {
    ...
    fn load_checkpoint(path: &Path) -> Result<Checkpoint> {
        let file = File::open(path).map_err(EraError::Io)?;
    ...
    pub fn save(&self) -> Result<()> {
        let tmp_path = self
            .checkpoint_path
            .with_extension(CHECKPOINT_TMP_EXTENSION.trim_start_matches('.'));
        ...
        // Write to temporary file
        {
            ...
            serde_json::to_writer_pretty(&mut writer, &self.checkpoint)
    ...
    pub fn update_file_progress(&mut self, bytes_added: u64, chunks_added: u32) -> Result<()> {
        ...
        // Don't save on every chunk update to reduce I/O
        Ok(())
    }
    ...
    pub fn record_chunk(&mut self, hash: ChunkHash, location: BlockLocation) {
        ...
        // Don't save on every chunk to reduce I/O
    }
...
mod tests {
    ...
    fn test_checkpoint_path_generation() {
        ...
        assert_eq!(
            checkpoint,
            PathBuf::from("/data/backups/archive.era.era.checkpoint")
        );

        let archive2 = Path::new("test.era");
        let checkpoint2 = Checkpoint::checkpoint_path(archive2);
        assert_eq!(checkpoint2, PathBuf::from("test.era.era.checkpoint"));
    }
    ...
    fn test_manager_atomic_save() {
        ...
        manager.record_chunk(hash, location);
        ...
        let tmp_path = manager
            .checkpoint_path
            .with_extension(CHECKPOINT_TMP_EXTENSION.trim_start_matches('.').to_string());
    ...
    fn test_checkpoint_serialization_roundtrip() {
...

        {
            ...
            manager.record_chunk(hash1, test_location(100, 50));
            manager.record_chunk(hash2, test_location(200, 100));
