...
fn test_precise_fault_tolerance_limits() {
    ...
    for volume_count in [3, 4, 6, 8].iter() {
        println!("\nTest Configuration: {} volumes, 4+2 erasure", volume_count);
        ...
        for volumes_to_lose in 1..=(*volume_count - 1) {
            ...
            for i in 0..*volume_count {
                ...
                let dst = if i == 0 {
                    ...
                    test_dir.path().join("test").with_extension(format!("era.{:03}", i))
                };
                if src.exists() {
                    fs::copy(&src, &dst).ok();
                }
            ...
            let Some(test_path) = open_path else {
                println!("  ❌ Lost {} volumes -> NO VOLUMES REMAINING", volumes_to_lose);
                continue;
            };
            ...
            match ArchiveReader::open(&test_path, "") {
                Ok(mut reader) => {
                    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                        reader.extract_all(&era_engine::ExtractOptions::new(test_dir.path().join("extract")))
                    }));
                    ...
                    match result {
                        ...
                        Err(_) => {
                            println!("  ❌ Lost {} volumes -> PANIC DURING EXTRACTION", volumes_to_lose);
                        }
                    }
                }
                Err(_) => {
...
fn test_shard_distribution_pattern() {
    ...
    for (volumes, total_shards) in configs {
        println!("Configuration: {} volumes, {} shards", volumes, total_shards);
